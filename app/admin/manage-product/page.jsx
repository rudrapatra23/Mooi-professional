...
export default function ManageProductsPage() {
  ...
  const fetchProducts = async ({ pageArg = page, limitArg = limit } = {}) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `/api/products?page=${pageArg}&limit=${limitArg}&ts=${Date.now()}`,
        {
          headers: {
      ...
      const items = Array.isArray(data?.items)
        ...
        ? data.products
        : [];
      const tot = Number.isFinite(Number(data?.total))
        ? Number(data.total)
        : items.length;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = await getToken();
      await axios.delete(
        `/api/product?productId=${encodeURIComponent(id)}&ts=${Date.now()}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Cache-Control": "no-cache",
    ...
    <div className="max-w-5xl mx-auto p-4">
      ...
      {error && (
        <div className="mb-4 text-red-600">
          Error: {error}
          <button
            onClick={() => fetchProducts({ pageArg: 1, limitArg: limit })}
          ...
          {sortedProducts.map((p) => (
            ...
            >
              ...
              <div className="flex-shrink-0 flex items-center gap-2">
                <button
                  onClick={() => openView(p)}
                  className="p-2 bg-blue-200 hover:bg-blue-300 rounded-full text-blue-700 transition"
                  title="View Product"
                >
                  <EyeIcon size={18} />
                </button>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="text-sm text-gray-500">
                ID: <span className="font-mono text-xs">{viewProduct.id}</span>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mt-4">
                  <div>
                    ...
                    <div className="font-medium">
                      ₹
                      {Number(
                        viewProduct.mrp ?? viewProduct.price ?? 0
                      ).toFixed(2)}
                    </div>
                  </div>
                  <div>
                ...
                <div className="mt-6 flex gap-2">
                  <button
                    onClick={() => copyToClipboard(viewProduct.id)}
                    className="px-3 py-1 bg-slate-200 rounded text-sm"
                  >
                    Copy ID
                  </button>
                  <Link
                    href={`/admin/edit-product/${viewProduct.id}`}
                    className="px-3 py-1 bg-yellow-200 rounded text-sm"
                  >
                    Edit
                  </Link>
                  ...
                  <button
                    onClick={closeView}
                    className="ml-auto px-3 py-1 bg-gray-100 rounded text-sm"
                  >
                    Close
                  </button>
                </div>
