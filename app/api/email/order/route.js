...
function createEmailHtml({
  ...
  const logoUrl = `${BASE}/logo.png`; // White background + black text logo
  const itemsHtml = itemsTableHtml(items);
  const total = Number(subtotal || 0) + Number(shipping || 0) + Number(gst || 0) - Number(discount || 0);
  const primaryColor = "#000000"; // Minimal black text

  // Polished copy for buyer; concise notice for seller
  const headingText = isBuyer ? "Order confirmed" : "New order received";
  const subText = isBuyer
    ? `Hi ${escapeHtml(buyerName || "Customer")}, thank you for shopping with <strong>Mooi Professional</strong>. Your order <strong>#${escapeHtml(orderId)}</strong> has been confirmed and is being prepared. We’ll email you again when the order ships with tracking information and an estimated delivery date.`
  ...
  // Buyer: no tracking button or extra CTAs. Seller: include admin button and login hint.
  const buyerActionHtml = ""; // intentionally empty for buyers
  const sellerActionHtml = `<a href="${BASE.replace(/^http:/, 'https:')}/admin/orders" style="display:inline-block;margin-top:22px;padding:12px 20px;background:#000;color:#fff;border-radius:6px;text-decoration:none;font-weight:600;">Open Admin Orders →</a>
    <div style="margin-top:10px;color:#666;font-size:13px;">Log in with the admin email: <strong>${escapeHtml(process.env.SELLER_EMAIL || "admin@example.com")}</strong></div>`;
  ...
  <head>
    ...
    <style>
      ...
      .wrap { max-width:680px; margin:40px auto; width:100%; }
      .card {
        background:#ffffff;
        border-radius:12px;
        overflow:hidden;
      ...
      @media (max-width:680px){
        .wrap{ width:100% !important; margin:0 auto !important; }
    ...
    <center>
      <div class="wrap">
        <div class="card">
          ...
          <div class="header">
            <img src="${escapeHtml(logoUrl)}" alt="Mooi Professional" style="height:48px; display:block; margin:0 auto;" />
          </div>

          <div class="body">
            <h2 style="margin:0 0 10px 0;font-size:22px;font-weight:700;color:${primaryColor};">${escapeHtml(headingText)}</h2>
            ...
            <div style="border-left:3px solid ${primaryColor}; padding-left:14px; margin-bottom:22px; background:#f9f9f9; padding:12px; border-radius:4px;">
              ...
              ${isBuyer
                ? `<p style="margin:0;font-size:14px;color:#444;">A confirmation has been sent to <a href="mailto:${escapeHtml(buyerEmail)}" style="color:${primaryColor};text-decoration:none;">${escapeHtml(buyerEmail)}</a></p>`
                : `<p style="margin:0;font-size:14px;color:#444;">Buyer: <strong>${escapeHtml(buyerName || "N/A")}</strong> — <a href="mailto:${escapeHtml(buyerEmail)}" style="color:${primaryColor};text-decoration:none;">${escapeHtml(buyerEmail)}</a></p>`
              }
            </div>

            <h3 style="margin-top:8px;margin-bottom:8px;color:#111;font-size:16px;font-weight:600;">Order Summary</h3>
            ${itemsHtml}
            ...
            <!-- Buyer: no CTA. Seller: show admin CTA + login hint. -->
            <div style="margin-top:18px;">
              ${isBuyer ? buyerActionHtml : sellerActionHtml}
            </div>

            <p style="margin-top:28px;color:#555;font-size:13px;line-height:1.6;">
              ${isBuyer
                ? `If you have any questions, reply to this email or contact us at <a href="mailto:${escapeHtml(process.env.SELLER_EMAIL || "info@mooiprofessional.com")}" style="color:${primaryColor};text-decoration:none;">${escapeHtml(process.env.SELLER_EMAIL || "info@mooiprofessional.com")}</a>.`
                : `For assistance, email <a href="mailto:${escapeHtml(process.env.SELLER_EMAIL || "info@mooiprofessional.com")}" style="color:${primaryColor};text-decoration:none;">${escapeHtml(process.env.SELLER_EMAIL || "info@mooiprofessional.com")}</a>.`
              }
            </p>
          </div>

          <div style="padding:20px;text-align:center;color:#999;font-size:13px;border-top:1px solid #f0f0f0;">
            &copy; ${new Date().getFullYear()} Mooi Professional — All rights reserved
          </div>
        </div>
      </div>
    </center>
...
export async function POST(req) {
  try {
    ...
    if (!buyerEmail || !orderId) {
      return new Response(JSON.stringify({ error: "Missing required fields (buyerEmail, orderId)" }), { status: 400 });
    }
    ...
    const buyerHtml = createEmailHtml({
      ...
      preheader: `Your Mooi Professional order ${orderId} is confirmed. Total: ${formatCurrency(totalAmount)}.`,
      orderId,
      buyerName,
      buyerEmail,
      items,
    ...
    if (debug.find(d => d.status === "error"))
      return new Response(JSON.stringify({ ok: false, debug }), { status: 502 });

    return new Response(JSON.stringify({ ok: true }), { status: 200 });
  } catch (err) {
    console.error("email/order error:", err);
    const details = err?.response?.body ?? err?.message ?? String(err);
    return new Response(JSON.stringify({ ok: false, error: "internal", details }), { status: 500 });
  }
}
