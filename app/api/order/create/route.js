...
export async function POST(req) {
  try {
    ...
    for (const it of rawItems) {
      ...
      if (!productId) return NextResponse.json({ ok: false, error: "invalid_item" }, { status: 400 });
    ...
    if (paymentMethod === "RAZORPAY") {
      ...
      const razorOrder = await razorpay.orders.create({
        amount: Math.round(result.order.total * 100), // in paisa
        currency: "INR",
      ...
      return NextResponse.json({
        ...
        keyId: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID, // frontend use
